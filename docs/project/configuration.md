...
| `traffic_monitor` | `object` | 见 schema | 当月流量统计和提醒设置。 |
...
- `traffic_monitor` 的统计基于系统网络计数器差值，首次启用只建立基线；系统重启或计数器重置期间的流量无法补记。
- 流量统计状态保存在 AstrBot 插件数据目录 `status_state.json` 的 `traffic_monitor` 命名空间中，仅保存当前自然月累计、上次采样基线和本月提醒状态。
